import { GridValueGetterParams } from '@mui/x-data-grid';
import { Story, Meta } from '@storybook/react';
import TableCellComponent from '../../Molecules/table-cell-component/table-cell-component';
import TableHeaderCell from '../../Molecules/table-header-cell/table-header-cell';
import DataGridV1, { IDataGridV1Props } from './data-grid-v1';
import { withDesign } from 'storybook-addon-designs';

export default {
  component: DataGridV1,
  title: 'Organisms/Data Grid V1',
  argTypes: {
    menuClicked: {
      action: 'clicked',
      table: {
        disable: true,
      },
    },
    onSearchInput: {
      action: 'clicked',
      table: {
        disable: true,
      },
    },
  },
  decorators: [withDesign],
} as Meta;

const Template: Story<IDataGridV1Props> = (args) => <DataGridV1 {...args} />;

export const FixedHeader = Template.bind({});
FixedHeader.args = {
  columnResizable: true,
  pagination: true,
  rowData: [
    {
      id: '1',
      status: { status: { color: '#037478', value: 'Active' } },
      name: {
        text: { value: 'Annetre Black1' },
        primaryAvatar: {
          label: { value: 'white' },
          path: 'data:image/jpeg;base64,',
        },
      },
      phoneNumber: { value: '(480)555-0103' },
      emailID: { value: 'annetreblack@gmail' },
      gender: { value: 'female' },
    },

    {
      id: '10',
      status: { status: { color: '#037478', value: 'Inactive' } },
      name: {
        text: { value: 'Robert Fox' },
        primaryAvatar: {
          label: { value: 'Robert Fox' },
          path: 'data:image/jpeg;base64,',
        },
        // tags: [
        //   {
        //     label: 'tag1',
        //     value: 'tag1',
        //   },
        //   {
        //     label: 'tag2',
        //     value: 'tag2',
        //   },
        //   {
        //     label: 'tag3',
        //     value: 'tag3',
        //   },
        //   {
        //     label: 'tag4',
        //     value: 'tag4',
        //   },
        // ],
      },
      phoneNumber: { value: '(480)555-0104' },
      emailID: { value: 'fox1@gmail.com' },
      gender: { value: 'male' },
    },
    {
      id: '11',
      status: { status: { color: '#037478', value: 'Inactive' } },
      name: {
        text: { value: 'Annetre Black' },
        primaryAvatar: {
          label: { value: 'Annetre Black' },
          path: 'data:image/jpeg;base64,',
        },
      },
      phoneNumber: { value: '(480)555-0104' },
      emailID: { value: 'fox@gmail.com' },
      gender: { value: 'male' },
    },
  ],
  columnData: [
    {
      field: 'id',
      headerName: 'ID',
      width: 200,
      sortable: false,
      renderCell: (values: any) => {
        return <TableCellComponent text={values}></TableCellComponent>;
      },
    },
    {
      headerName: 'Name',
      field: 'name',
      width: 200,
      editable: false,
      sortable: false,

      renderCell: (values: any) => {
        return (
          <TableCellComponent
            text={values.value.text}
            status={values.value.status}
            primaryAvatar={values.value.primaryAvatar}
            leftAvatar={values.value.leftAvatar}
            rightAvatar={values.value.rightAvatar}
            tags={values.value.tags}
          ></TableCellComponent>
        );
      },
    },

    {
      headerName: 'Status',
      field: 'status',
      width: 200,
      editable: false,
      sortable: false,

      renderCell: (values: any) => {
        return (
          <TableCellComponent
            text={values.value.value}
            status={values.value.status}
            primaryAvatar={values.value.primaryAvatar}
            leftAvatar={values.value.leftAvatar}
            rightAvatar={values.value.rightAvatar}
            tags={values.value.tags}
          ></TableCellComponent>
        );
      },
    },
    {
      headerName: 'Phone Number',
      field: 'phoneNumber',
      width: 200,
      editable: false,
      sortable: false,

      renderCell: (values: any) => {
        return (
          <TableCellComponent
            text={values.value}
            status={values.value}
            primaryAvatar={values.value.primaryAvatar}
            leftAvatar={values.value.leftAvatar}
            rightAvatar={values.value.rightAvatar}
            tags={values.value.tags}
          ></TableCellComponent>
        );
      },
    },
    {
      headerName: 'Email Id',
      field: 'emailID',
      width: 200,
      editable: false,
      sortable: false,

      renderCell: (values: any) => {
        return <TableCellComponent text={values.value}></TableCellComponent>;
      },
    },
    {
      headerName: 'Gender',
      field: 'gender',
      width: 200,
      editable: false,
      sortable: false,

      renderCell: (values: any) => {
        return <TableCellComponent text={values.value}></TableCellComponent>;
      },
    },
  ],
};
FixedHeader.parameters = {
  design: {
    type: 'figma',
    url: 'https://www.figma.com/proto/ByIvCrteT7rhSlgkX1wYxE/IAURO---DSL?node-id=294%3A69647&scaling=min-zoom&page-id=142%3A5528&starting-point-node-id=294%3A41952',
  },
};

export const ColumnResizableInCommercialVersion = Template.bind({});
ColumnResizableInCommercialVersion.args = {
  columnResizable: true,
  pagination: true,
  rowData: [
    {
      id: '1',
      status: { status: { color: '#037478', value: 'Active' } },
      name: {
        text: { value: 'Annetre Black1' },
        primaryAvatar: {
          label: { value: 'white' },
          path: 'data:image/jpeg;base64,',
        },
      },
      phoneNumber: { value: '(480)555-0103' },
      emailID: { value: 'annetreblack@gmail' },
      gender: { value: 'female' },
    },

    {
      id: '10',
      status: { status: { color: '#037478', value: 'Inactive' } },
      name: {
        text: { value: 'Robert Fox' },
        primaryAvatar: {
          label: { value: 'Robert Fox' },
          path: 'data:image/jpeg;base64,',
        },
        // tags: [
        // {
        //   label: 'tag1',
        //   value: 'tag1',
        // },
        // {
        //   label: 'tag2',
        //   value: 'tag2',
        // },
        // {
        //   label: 'tag3',
        //   value: 'tag3',
        // },
        // {
        //   label: 'tag4',
        //   value: 'tag4',
        // },
        // ],
      },
      phoneNumber: { value: '(480)555-0104' },
      emailID: { value: 'fox1@gmail.com' },
      gender: { value: 'male' },
    },
    {
      id: '11',
      status: { status: { color: '#037478', value: 'Inactive' } },
      name: {
        text: { value: 'Annetre Black' },
        primaryAvatar: {
          label: { value: 'Annetre Black' },
          path: 'data:image/jpeg;base64,',
        },
      },
      phoneNumber: { value: '(480)555-0104' },
      emailID: { value: 'fox@gmail.com' },
      gender: { value: 'male' },
    },
  ],
  columnData: [
    {
      field: 'id',
      headerName: 'ID',
      width: 200,
      sortable: false,
      renderCell: (values: any) => {
        return <TableCellComponent text={values}></TableCellComponent>;
      },
    },
    {
      headerName: 'Name',
      field: 'name',
      width: 200,
      editable: false,
      sortable: false,

      renderCell: (values: any) => {
        return (
          <TableCellComponent
            text={values.value.text}
            status={values.value.status}
            primaryAvatar={values.value.primaryAvatar}
            leftAvatar={values.value.leftAvatar}
            rightAvatar={values.value.rightAvatar}
            tags={values.value.tags}
          ></TableCellComponent>
        );
      },
    },

    {
      headerName: 'Status',
      field: 'status',
      width: 200,
      editable: false,
      sortable: false,

      renderCell: (values: any) => {
        return (
          <TableCellComponent
            text={values.value.value}
            status={values.value.status}
            primaryAvatar={values.value.primaryAvatar}
            leftAvatar={values.value.leftAvatar}
            rightAvatar={values.value.rightAvatar}
            tags={values.value.tags}
          ></TableCellComponent>
        );
      },
    },
    {
      headerName: 'Phone Number',
      field: 'phoneNumber',
      width: 200,
      editable: false,
      sortable: false,

      renderCell: (values: any) => {
        return (
          <TableCellComponent
            text={values.value}
            status={values.value}
            primaryAvatar={values.value.primaryAvatar}
            leftAvatar={values.value.leftAvatar}
            rightAvatar={values.value.rightAvatar}
            tags={values.value.tags}
          ></TableCellComponent>
        );
      },
    },
    {
      headerName: 'Email Id',
      field: 'emailID',
      width: 200,
      editable: false,
      sortable: false,

      renderCell: (values: any) => {
        return <TableCellComponent text={values.value}></TableCellComponent>;
      },
    },
    {
      headerName: 'Gender',
      field: 'gender',
      width: 200,
      editable: false,
      sortable: false,

      renderCell: (values: any) => {
        return <TableCellComponent text={values.value}></TableCellComponent>;
      },
    },
  ],
};
ColumnResizableInCommercialVersion.parameters = {
  design: {
    type: 'figma',
    url: 'https://www.figma.com/proto/ByIvCrteT7rhSlgkX1wYxE/IAURO---DSL?node-id=294%3A69647&scaling=min-zoom&page-id=142%3A5528&starting-point-node-id=294%3A41952',
  },
};
