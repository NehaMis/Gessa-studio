...
export function Transfer(props: TransferProps) {
  ...
  const [leftSearchValue, setLeftSearchValue] = React.useState<string>();
  const [rightSearchValue, setRightSearchValue] = React.useState<string>();
  ...
  const leftGridDataHandler = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const filteredList = left.filter((items) =>
      items?.value?.toString().includes(e.target.value)
    );
    console.log(filteredList);
    setLeftFiltered(filteredList);
  };
